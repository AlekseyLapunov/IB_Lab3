...
void MainWindow::reactToKeyLength()
{
    if(ui->keyLine->text().length() < 16 )
    {
        ...
        ui->keyCountLabel->setStyleSheet("QLabel { background-color : none; color : red; }");
    }
    else
    {
        ui->rememberKeyButton->setDisabled(false);
        ui->keyCountLabel->setStyleSheet("QLabel { background-color : none; color : black; }");
    }
    ui->keyCountLabel->setText(QString::number(ui->keyLine->text().length()) + " / 16");
}
...
void MainWindow::toggleWidgets(bool toggle)
{
    ...
    ui->keyCountLabel->setEnabled(toggle);
